...
function client.openInventory(inv, data)
	...
	if canOpenInventory() then
		...
		if left then
			...
			if client.screenblur and shared.gameVersion == 'fivem' then TriggerScreenblurFadeIn(0) end
			closeTrunk()
...
RegisterNetEvent('ox_inventory:closeInventory', function(server)
	if invOpen then
		...
		if not shared.gameVersion == 'redm' then
...
function DeleteDropObject(entity)
	if entity then
		DeleteObject(entity)
	end
end

function CreateDropObject(model, pos)
	...
	if IsModelInCdimage(hash) then
        ...
        local entity = CreateObject(hash, pos.x, pos.y, pos.z + 1, isNetworked, false, false)

        if entity == 0 then
            print("ERROR: Could not create object", hash)
            return nil
        end

        Citizen.InvokeNative(0x7C348310A6E2FB91, entity, true)
		...
		print("ERROR: Model hash not in game ", hash)
	end
end
...
local function nearbyDrop(self)
	if not self.instance or self.instance == currentInstance then
		...
		local entityHeight = GetEntityHeightAboveGround(entityHeight)

		Citizen.InvokeNative(
			0x2A32FAA57B937173, 
			0x94FDAE17, 
			self.coords.x, self.coords.y, self.coords.z - entityHeight,
			0, 0, 0,
			0, 0, 0,
			1.0, 1.0, 1.0,
			250, 250, 255,
	...
	client.tick = SetInterval(function(disableControls)
		...
		if invOpen then
			...
			elseif Citizen.InvokeNative(0x580417101DDB492F, 0, keys[2]) then
				ExecuteCommand('hotkey2')	
			elseif Citizen.InvokeNative(0x580417101DDB492F, 0, keys[3]) then
				ExecuteCommand('hotkey3')	
			elseif Citizen.InvokeNative(0x580417101DDB492F, 0, keys[4]) then
				ExecuteCommand('hotkey4')	
			elseif Citizen.InvokeNative(0x580417101DDB492F, 0, keys[5]) then
				ExecuteCommand('hotkey5')	
			end
		
			if Citizen.InvokeNative(0x580417101DDB492F, 0, 0xE30CD707) then
